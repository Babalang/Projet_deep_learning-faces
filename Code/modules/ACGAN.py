...
def load_fer2013_dataset():
    data = pd.read_csv(BASE / 'data/fer2013.csv')
...
def run_cgan():
    ...
    save_model(d_model, BASE / 'discriminator_model.h5')  # Save discriminator model
    # Save generator model
    save_model(g_model, BASE / 'generator_model.h5')  # Save generator model
    # Save GAN model (including both generator and discriminator)
    save_model(gan_model, BASE / 'acgan_model.h5')  # Save CGAN model


def generate_emotion_image(latent_vector, emotion_label):
    g_model = load_model(BASE / 'generator_model.h5', compile=False)
...
def train_vae2gan_adapter(epochs=30, batch_size=64, vae_latent_dim=256, gan_latent_dim=128):
    ...
    adapter_path = BASE / 'vae2gan_adapter.h5'
...
def invert_latent_acgan(img_path, latent_dim=128, steps=800, lr=0.02, n_starts=5):
    import cv2
    g_model = load_model(BASE / 'generator_model.h5', compile=False)
    d_model = load_model(BASE / 'discriminator_model.h5', compile=False)
...
def display_gan():
    ...
    choice = input("Entrez '1. generate' pour générer une image, '2. modify' (sans adaptateur), '3. modify_adapter' ou '4. modify_invert': ").strip().lower()
    if choice == 'generate' or choice == '1':
...
def display_training():
    ...
    choice = input("Entrez '1. train' pour entraîner le GAN, '2. adapt' pour entraîner l'adaptateur: ").strip().lower()
    if choice == 'train' or choice == '1':
        run_cgan()
    elif choice == 'adapt' or choice == '2':
        train_vae2gan_adapter(epochs=30, batch_size=32, vae_latent_dim=256, gan_latent_dim=128)
    else:
        print("Choix invalide. Veuillez entrer 'train' ou 'adapt'.")
