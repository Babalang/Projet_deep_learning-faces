...
def load_model(
    ...
    # ----------------------------
    if load_weights:
        model = weight_utils.load_model_weights(model=model, weights_file=BASE / "emotion_model.h5")
...
def train_emotion_model():
    ...
    X_train,y_train = load_fer2013_dataset(DATA_DIR / "train",labels)
    print("Loading validation dataset...")
    X_test,y_test = load_fer2013_dataset(DATA_DIR / "test",labels)
    ...
    model.save(BASE / "emotion_pretrained.h5")

    # 2. Fine-tuning avec class_weight
    model = load_model(load_weights=False)
    model.load_weights(BASE / "emotion_pretrained.h5")
    model.compile(optimizer='adam', loss='categorical_crossentropy', metrics=['accuracy'])
    ...
    model.save(BASE / "emotion_model.h5")
...
def vae_train():
    X_train, y_train = load_faces_dataset_with_labels(DATA_DIR / "train", labels)
    ...
    encoder.save(BASE / "encoder_model.h5")
    decoder.save(BASE / "decoder_model.h5")
    encoder.save_weights(BASE / "vae_encoder_weights.h5")
    decoder.save_weights(BASE / "vae_decoder_weights.h5")